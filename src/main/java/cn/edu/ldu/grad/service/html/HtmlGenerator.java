...
public class HtmlGenerator {
  ...
  public String getStudentDoc(String userId) throws IOException {
    Resource htmlResource = new ClassPathResource("pdf/html_template/studentdoc.html");
    File htmlFile = htmlResource.getFile();
    BufferedReader bufferedReader =
        new BufferedReader(new InputStreamReader(new FileInputStream(htmlFile)));
    ...
    StudentDocInfo docInfo = studentDocInfoMapper.selectByPrimaryKey(userId);
    String html = stringBuilder.toString();
    html = html.replace("$year$", DateTime.now().getYear() + "").replace("$xm$", docInfo.getXm())
        .replace("%userId%", docInfo.getZjhm()).replace("$zy$", docInfo.getLqzymc())
        .replace("$xy$", docInfo.getLqyxsmc()).replace("$xxfs$", docInfo.getLqxxfsmc())
        .replace("$xz$", docInfo.getXz()).replace("$dadw$", docInfo.getDaszdw())
        .replace("$date$",
            DateTime.now().getYear() + "年" + DateTime.now().getMonthOfYear() + "月"
                + DateTime.now().getDayOfMonth() + "日")
        .replace("$xh$", docInfo.getXhtxm()).replace("$xy$", docInfo.getLqyxsmc())
        .replace("$xh0$", docInfo.getXh()).replace("$userId$", docInfo.getZjhm());
    return html;
  }
  ...
  public String getHtml(String bmh) throws IOException {
    ...
    String html = stringBuilder.toString();
    html = html// .replace("[#port#]",environment.getProperty("server.port"))
               // //titlePicFile.getPath().replace("\\","/"))
        .replace("[#year#]", DateTime.now().getYear() + "").replace("[#picno#]", picNo)
        .replace("[#xm#]", examResultInfo.getXm()).replace("[#ksbh#]", examResultInfo.getKsbh())
